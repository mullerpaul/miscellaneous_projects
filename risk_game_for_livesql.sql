...
CREATE OR REPLACE PACKAGE BODY risk_simulation IS
  ...
  PROCEDURE simulate_battle(pi_attack_armies          IN NUMBER,
    ...
    lc_simulation_runs CONSTANT NUMBER := 500;  --2000 is too long for liveSQL
...
BEGIN
  risk_simulation.simulate_battle(pi_attack_armies => 3, pi_defend_armies => 1);  --should be 5/12 (41%)
END;
/
...
select * 
  from risk_statistics_rollup_vw
